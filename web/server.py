    ...
    for p in positions:
        ...
        if value >= 20000:
            ...
            dust_positions.append({
                "asset": sym, "qty": p.quantity, "price": current_price, "value": value
            })
    
    # Add positions from exchange balances if DB positions are empty
    if not pos_list:
        try:
            from client.rest import TokocryptoClient
            client = TokocryptoClient()
            ...
            ticker_prices = {p["symbol"]: float(p["lastPrice"]) for p in prices_24h}
            ...
            for b in balances:
                ...
                cp = ticker_prices.get(price_key, 0)
                
                if cp == 0:
                    for p_key, p_val in ticker_prices.items():
                        if b.asset.lower() in p_key.lower() and 'IDR' in p_key.upper():
                            cp = p_val
                            break
                
                value = b.free * cp
                if cp > 0 and value >= 20000:
                    pos_list.append({
                        "symbol": f"{b.asset}_IDR", "side": "BUY",
                        "entry_price": 0, "quantity": b.free,
                        "current_price": cp,
                        "pnl": 0, "pnl_pct": 0,
                        "stop_loss": 0, "take_profit": 0,
                    })
                elif cp > 0 and value > 0:
                    dust_positions.append({
                        "asset": b.asset, "qty": b.free, "price": cp, "value": value
                    })
        except Exception as e:
            logger.warning(f"Failed to fetch exchange positions: {e}")
    
    # Always add dust positions from exchange balances
    try:
        from client.rest import TokocryptoClient
        client = TokocryptoClient()
        balances = client.get_account_info()[0]
        ...
        for b in balances:
            ...
            value = b.free * cp
            existing_sym = f"{b.asset}_IDR"
            existing = any(p['symbol'] == existing_sym for p in pos_list)
            if cp > 0 and value > 0 and value < 20000 and not existing:
                dust_positions.append({
                    "asset": b.asset, "qty": b.free, "price": cp, "value": value
                })
    except Exception as e:
...
async def api_trades(symbol: str = "", strategy: str = "", side: str = "", limit: int = 100, sync: int = 1):
    ...
    if not side and not symbol:
        ...
        for sym in symbol_trades:
            symbol_trades[sym].sort(key=lambda x: x["trade_time"])
        ...
        for sym, sym_trades in symbol_trades.items():
            ...
            for t in sym_trades:
                if t["side"] == "BUY":
                    buys.append({"qty": t["qty"], "price": t["price"], "trade_time": t["trade_time"], "commission": t.get("commission", 0), "orig_trade": t})
                    result.append({
                        "trade_id": f"{sym}_BUY_{t['trade_time']}",
                        "symbol": sym,
                        "side": "BUY",
                        "price": t["price"],
                        "qty": t["qty"],
                    ...
                    result.append({
                        "trade_id": f"{sym}_SELL_{t['trade_time']}",
                        "symbol": sym,
                        "side": "SELL",
                        "price": sell_price,
                        "qty": t["qty"],
