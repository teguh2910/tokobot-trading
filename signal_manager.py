    ...
    def close_position(self, position: Position, reason: str):
        if config.BOT_MODE == "paper":
            ...
            try:
                ...
                actual_qty = balance.free if balance else 0
                ...
                if qty * position.current_price >= min_notional:
                    ...
                    dust_value = int(qty * position.current_price)
                    logger.info(f"Try quoteOrderQty for {position.symbol} dust cleanup (val={dust_value} IDR)")
                    order = self.client.new_order(
                        symbol=position.symbol,
                        side=side,
                        order_type=2,
                        quote_order_qty=str(dust_value),
                    )
                ...
                sold_qty = order.executed_qty if order.executed_qty > 0 else qty
                pnl = (close_price - position.entry_price) * sold_qty
                if position.side == "SELL":
                    pnl = (position.entry_price - close_price) * sold_qty
                ...
                trade = Trade(
                    ...
                    price=close_price,
                    qty=sold_qty,
                    quote_qty=close_price * sold_qty,
                    commission=0,
                    commission_asset="",
                    is_buyer=(side == 0),
                    is_maker=False,
                ...
                save_trade(trade, pnl=pnl, strategy=position.strategy)
                remove_position(position.symbol, position.side)
                logger.info(f"[LIVE] Closed {position.symbol} {position.side} @ {close_price:.2f} qty={sold_qty:.6f} PnL={pnl:.2f} | {reason}")
            except Exception as e:
                err = str(e)
